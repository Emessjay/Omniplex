...
### Load-bearing decisions (per-feature)

The detailed "load-bearing decisions from `<feature>`" blocks that workers
accrete have been moved out of this file to keep it lean. They live in
[`context/`](context/), split by area (see [`context/README.md`](context/README.md)
for the index): foundation, commands/UI, economy, universe-generation, travel,
survival/combat, production/bases, ships, factions/social, exploration.

**Workers: append new load-bearing-decision blocks to the appropriate
`context/<category>.md` file** (and add a line to `context/README.md` if you
introduce a new category), NOT to this file.
